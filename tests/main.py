...
parser.add_argument (
        ...
        help = "Specifies which tests will be run. Each test requires a different set of parameters. Before running each scenario ensure that an admin user is stored in the databasei. Following scenarios are supported:\n" \
               ...
               "5) Value \"vote\" is used for running tests which grade endpoints for admin and retrieving election results. For these tests you can specify the following parameters:\n" \
               "\t--administrator-address\n" \
               "\t--station-address\n" \
               "\t--with-authentication (if this is not present the penalty is 0.1 * score)\n" \
               "\t--authentication-address (if --with-authentication is present this argument is used to specify the address of the authentication container)\n" \
